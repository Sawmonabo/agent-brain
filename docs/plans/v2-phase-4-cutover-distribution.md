...
> **SUPERSEDED AS-BUILT (T4.5 execution, 2026-07-10, controller ruling + review-verified):** the probe source "newest agb1-blob reachable from HEAD" was internally inconsistent with this task's own mandated e2e assertion. On exactly the machine this check exists to warn — a stale-keyset machine after a fleet rotation — integrate's all-or-nothing invariant freezes HEAD at the last fully-decryptable commit (the `--abort` ladder, T4), while `git fetch` (integrate.go:48, unconditional, before the rebase) still advances `refs/remotes/origin/main` to the re-encrypted tip. A HEAD-only probe therefore keeps decrypting cleanly on the degrading machine and the B-side Warn assertion is unsatisfiable as written. As-built: `probeUpstreamRef` resolves `refs/remotes/origin/main` FIRST and falls back to HEAD (fresh init / no remote-tracking ref). Still offline-safe — the remote-tracking ref is a local ref read via `git rev-parse`, zero network. Verified independently by the task review (engine-code walk; both pinning tests — package-level `TestRunKeysetDecryptStaleKeysetWarns` and the e2e B-side — re-run green). Files as-built additionally include `internal/cli/dashboard/{doctorview.go,dashboard.go,doctorview_test.go}` (new `StatusInfo` enum value needs glyph + counter arms in the exhaustive switches — accepted ripple) and `internal/cli/filter_test.go` (commitRawBlob test-helper `--no-filters` correctness fix).
...
> **SUPERSEDED AS-BUILT (T6.5 execution, 2026-07-10, controller rulings + review-verified):** two corrections. **(D1 — LastCycle sourcing:** the Step-1 branch pair below was incomplete — `engine.Report` has no per-unit row AND structurally cannot carry a whole-cycle "error": that outcome is `Sync`'s `error` return (sync.go:17), seen only by the daemon. Growing `Report` (branch b) would be redundant with `Report.Degraded` and still inexpressive. As-built, `LastCycle` is DERIVED AT THE DAEMON in `recordOutcome` — precedence error > degraded > ok, keyed per folder (the sync unit), stamped from `summary.At`; only folders synced that cycle update, and the nil-synced registry-failure path leaves history untouched. Review verified the derivation has no holes.) **(C2 — WatchTriggers semantics:** ADR 07 makes triggers fleet-global — `Trigger` carries no root — so "per-unit trigger counts" as literally specced cannot exist. As-built: per-root PARTICIPATION counts (each fs/overflow trigger increments every currently-watched root; the watcher's "poll" backstop is excluded so the number reads as filesystem activity), and Activity's fleet line shows the **MAX over units, not the sum below** — sum would amplify by fleet size under a "watch triggers" label (14 triggers × 3 units would render 42); max = the raw trigger count since the longest-watched root, and incidentally removes the shared-LocalDir double-count sum had.) Also absorbed in the review fix round: `recordOutcome`'s degraded-flag rebuild is scoped to synced folders — the inherited wholesale wipe silently marked unsynced folders healthy on filtered `sync --project` cycles and cleared ALL flags on the registry-failure path (pre-existing defect made user-visible by LastCycle; fixed with a discriminating RED-first test).
...
### Task 11: main becomes the scrubbed develop line (REVISED 2026-07-12 — merge dropped; USER-GATED, folds into Task 12)

**Original shape (superseded):** `git merge --no-ff develop` into main. Dropped
by the ADR-13 amendment (2026-07-12): the scrub now erases ALL pre-greenfield
(v1) history, and a merge commit would tie that erased graph back into main.
develop already carries main's shared root, so merging adds no content — only
the v1 lineage this cutover exists to remove.

**Revised shape:** there is no merge step. After Task 12's graft + filter-repo,
the scrubbed develop line is pushed as `main` on the recreated repo instance
(12.3), and `develop` is recreated from it. ADR 11's *gate* is unchanged —
main changes only when v2 demonstrably works end-to-end (Tasks 9–10 complete,
soak clean, cutover table full); only the vehicle changed from merge commit to
branch replacement.

- [ ] **11.1** Preconditions re-check (the old merge preconditions, verbatim): Tasks 9–10 verified on every inventoried machine; soak clean; ledger's cutover table complete.
- [ ] **11.2** Reconcile local main vs origin/main (recon 2026-07-10: local ahead 4 docs / behind 3 bash-era ops — all v1-era; adjudicate deliberately, never blind pull) so the pre-scrub mirror archive captures everything worth archiving. Nothing from old main enters the scrubbed history.
- [ ] **11.3** Full suite foreground + lint + linux cross-compile at the develop tip that will become main (same commands as exit criteria C1–C3); ledger entry. Proceed to Task 12 in the same sitting or later — 12 does the actual branch replacement.

### Task 12: ADR-13 history scrub + age-key retirement + v2.0.0 (USER-GATED, DESTRUCTIVE — the point of no return)

> **As-executed postscript (2026-07-17):** 12.1–12.3 are done — scrub verified,
> repo deleted and recreated PUBLIC, scrubbed line pushed as `main`+`develop`.
> Deviations from the text below, recorded in ADR 13's execution record and
> ADR 16 decisions 6–7: `--sensitive-data-removal` was dropped (it re-mirrors
> origin refs and discards the graft; replaced by strip-to-one-local-ref +
> plain filter-repo + separate `--invert-paths` passes); `docs/archive/` was
> erased too; the first tag ships as **`v1.0.0`**, not `v2.0.0` (suffixless
> module path; public history starts at v1); tap push authenticates via a
> per-run GitHub App token, not a PAT. 12.4–12.6 (age key, first tag, backups)
> proceed on their recorded schedule.
...
- [ ] **12.1 Fresh clone + graft + filter (REVISED 2026-07-12 — full v1 erasure per the ADR-13 amendment):** `git clone git@github.com:Sawmonabo/agent-brain.git /tmp/scrub && cd /tmp/scrub`, then `git replace --graft <greenfield-reset-sha>` (parentless — the reset commit becomes the root; resolve its post-scrub-era sha in the clone by subject: `git log --format='%H %s' develop | grep 'greenfield reset'`), then `git filter-repo --force --sensitive-data-removal --invert-paths --path home/dot_agent-brain` (git-filter-repo v2.47.0 via brew/pipx; the run bakes the graft into real history). Result: every pre-greenfield commit — all v1 code history and all `memory: <host> <timestamp>` flush commits — ceases to exist; the path filter stays as belt-and-braces. Optional: rewrite the root commit's message via filter-repo's message callback so it does not reference the replaced system.
- [ ] **12.2 Verify BEFORE any push (ADR 13):** `gitleaks git --no-banner --log-opts=--all .` over the scrubbed clone → clean (**`--log-opts=--all` is load-bearing**: a bare scan covers only the current branch's log — verified 2026-07-09 when develop-only scanning came back clean while main's bash-era `.age` blobs sat unscanned on another ref); `git log --all --oneline -- home/dot_agent-brain` → EMPTY; `git log --all --format=%s | grep -c '^memory:'` → 0; **`git log --all --reverse --oneline | head -3` → the greenfield-reset commit is the root, zero commits older than it**; spot-inspect `git log --all --stat | head -100`; size sanity (`git count-objects -vH` shrunk vs the archive).
- [ ] **12.3 Delete-and-recreate (the chosen finish — GitHub retains cached views + unreachable objects past force-pushes):** `gh repo delete Sawmonabo/agent-brain --yes` (the `delete_repo` scope was added 2026-07-09) → `gh repo create Sawmonabo/agent-brain --private --description ...` (**or `--public`** — the zero-cost option is live the moment the scrub verifies; USER DECIDES here, recorded in the ledger) → from the scrubbed clone: `git remote add origin git@github.com:Sawmonabo/agent-brain.git`, push the scrubbed line as `main` plus a `develop` recreated from it (`git push origin HEAD:main HEAD:develop`). **Do NOT `git push --tags`** — the rc tags reference pre-scrub history and die with the old instance (accepted casualties, recorded 11.3-old); `v2.0.0` becomes the fresh instance's first tag at 12.5. Re-point/re-clone every machine's working copy (`git remote set-url` is NOT enough — SHAs changed; fresh clone, carry over uncommitted work manually if any).
...
- [ ] **12.5 v2.0.0 final:** `git tag v2.0.0 && git push origin v2.0.0` from main on the NEW repo instance → release workflow → four archives + checksums; if 12.3 chose public, the formula publishes and `brew install sawmonabo/tap/agent-brain` goes live — verify on a machine (`brew install` + `agent-brain --version`, which also live-proves the ad-hoc signature end-to-end); if private, the gh-download path remains and the formula activates whenever the repo flips public (re-run the tag's workflow or cut v2.0.1). Fleet machines take it with bare `agent-brain update` (rc.5+ binaries; an rc.4-or-older binary needs `--prerelease` for that one hop).
- [ ] **12.6 Local backup retirement (LAST, after a post-scrub soak — added 2026-07-12):** the plaintext quarantine (`~/.agent-brain-quarantine-20260710`: byte-verified live-store copies, the Jul-8 audit prunes, and `ai-sidekicks-backup-pre-audit-cleanup.tar.gz`) and the legacy chezmoi checkout (`~/dev/agent-brain-legacy`) outlive the scrub as the last v1 fallbacks. Delete them only after: fleet migration verified on every machine (10.5), scrub verified (12.2), and the first post-scrub weeks pass alongside the mirror archive's retention window. The tarball is the ONLY copy of the deliberately-pruned memories — decide explicitly: vault-stash the one 145K file or accept forgetting them. Ledger the disposition.
...
12. main replaced by the scrubbed develop line (merge dropped per the ADR-13 amendment 2026-07-12); suite green at the tip that became main (Task 11 revised).
13. History scrub executed + verified (gitleaks-clean, zero bash-era paths/commits, greenfield-reset commit is the root); delete-and-recreate done; every clone refreshed; age key retired everywhere (Task 12).
...
17. **Scrub scope: full v1 erasure; merge-to-main dropped (2026-07-12, Sawmon — recorded in the ADR-13 amendment).** Trigger: Sawmon's cutover goal is "history looks like v1 never existed; v2.0.0 is the new baseline", and the memories-only scrub leaves bash-era code commits visible. Mechanism: graft the greenfield-reset commit as the new root (`git replace --graft` + filter-repo bakes it in), path filter retained as belt-and-braces; develop already shares old main's root (verified: 50 commits touching `home/dot_agent-brain`, 48 `memory:` subjects reachable from develop), so a --no-ff merge would only re-tie the erased lineage — the scrubbed develop line becomes main instead (T11 revised; ADR 11's demonstrated-end-to-end gate unchanged). Old rc tags are not re-pushed. Local plaintext backups (quarantine + legacy checkout) are deleted LAST (new 12.6), after fleet verification + post-scrub soak; migration fidelity pre-verified on this Mac 2026-07-12 (ai-sidekicks live store 68/68 byte-identical to the migrate seed).
