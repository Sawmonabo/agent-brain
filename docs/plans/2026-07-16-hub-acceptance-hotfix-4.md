...
Applied after the final whole-branch review; each item is WHAT changed and WHY.

- **Re-auth return always re-probes; the launch toast is reserved for genuine launch failures.** `tea.ExecProcess` delivers the child's `cmd.Run()` error to the finish message, so `gh`'s own non-zero exit (an `*exec.ExitError` — e.g. the user ctrl-C's the device flow, exit 130, yet may have fixed the token in the browser) arrives there exactly as a launch failure would. The finish handler now runs the post-handoff `gh auth status` probe on every return — the probe is the sole truth about whether the token is live — and shows the sticky "gh auth login did not run" toast only when the error is present and is NOT an `*exec.ExitError` (the handoff never started). A clean exit and a non-zero exit both converge through the probe; the nil-probe build still takes its honest probe-unavailable branch, and the 1007 re-assert stays unconditional. The `ghAuthFinishedMsg` and handler doc comments now state this delivery contract instead of the earlier "err is a launch failure, not gh's exit code" claim.
- **Doc/spec truth alignments.** (1) The daemon's conflict-log export comment no longer says the dashboard conflicts view is deferred — the hub's conflict center ships and reads that file alongside the `agent-brain conflicts` CLI. (2) ADR 21's Consequences no longer claims Activity/Doctor "gain nothing today"; their scrolling landed, and the bullet now records that these cursorless tab panes leave `↑/↓` unbound, so the alternate-scroll wheel does not move them while the explicit `ctrl+d/u`/`pgup/pgdn`/`g/G` keys do. (3) The tab-body scroll convention is now documented in spec §2 (keys, pinned section title, overflow hint, reset-on-identity-change, the deliberately-unbound wheel), and `views/scrollpane.go`'s citation points there instead of the unrelated §7. (4) Decision 10's marker-placement clause is no longer stated as universal ("before the exit, never after it"): the marker renders at the elision point, which falls before the trailing exit rows in the list scope and after the leading exit row in the focused scope (`esc/tab list · … ?`), matching `fitFooterSegments`' own conditional doc. (5) Spec §2's status-bar enumeration no longer lists "version" — the build version renders on the Projects fleet header (§9), not the status bar; the two code comments that quoted the old schematic were aligned to match.
