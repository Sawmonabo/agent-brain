...
func TestMain(m *testing.M) {
	if len(os.Args) > 1 {
		...
		case "git-clean", "git-smudge", "git-textconv", "git-merge":
			fmt.Fprintln(os.Stderr, "daemon.test invoked as a git filter — a fixture wired filter config at the test binary, which would recurse the whole suite as its own clean/smudge driver; see testBinaryPath's doc comment, this file")
			os.Exit(1)
		}
	}
	os.Exit(testMain(m))
